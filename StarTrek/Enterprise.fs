...
let setDevice enterprise deviceNo value =
    ...
    | 1 ->
        { enterprise with
            WarpEngines = enterprise.WarpEngines + value }
    | 2 ->
        { enterprise with
            ShortRangeSensors = enterprise.ShortRangeSensors + value }
    | 3 ->
        { enterprise with
            LongRangeSensors = enterprise.LongRangeSensors + value }
    | 4 ->
        { enterprise with
            ShieldControl = enterprise.ShieldControl + value }
    | 5 ->
        { enterprise with
            LibraryComputer = enterprise.LibraryComputer + value }
    | 6 ->
        { enterprise with
            PhaserControl = enterprise.PhaserControl + value }
    | 7 ->
        { enterprise with
            PhotonTubes = enterprise.PhotonTubes + value }
    | 8 ->
        { enterprise with
            DamageControl = enterprise.DamageControl + value }
    | _ -> enterprise
